using System;
using System.Collections.Generic;

namespace Common.Database.Entities;

public partial class DbAoNuoiNhapMt
{
    public string Ma { get; set; } = null!;

    public string AoNuoiMa { get; set; } = null!;

    public DateOnly Ngay { get; set; }

    public decimal NhietDo { get; set; }

    public decimal PhSang { get; set; }

    public decimal PhChieu { get; set; }

    public decimal O2Sang { get; set; }

    public decimal O2Chieu { get; set; }

    public decimal Nh3 { get; set; }

    public decimal H2s { get; set; }

    public decimal Kh { get; set; }

    public string? Note { get; set; }

    public int RowStatus { get; set; }

    public DateTime CreatedAt { get; set; }

    public string CreatedBy { get; set; } = null!;

    public DateTime UpdatedAt { get; set; }

    public string UpdatedBy { get; set; } = null!;

    public virtual UsUser CreatedByNavigation { get; set; } = null!;

    public virtual UsUser UpdatedByNavigation { get; set; } = null!;
}
